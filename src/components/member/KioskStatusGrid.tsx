...
export default function KioskStatusGrid({ refreshKey }: Props) {
  ...
  const fetchSlots = async () => {
    ...
    const { data } = await supabase
      .from("kiosk")
      .select("compartment, book_id, book_name")
      .order("compartment");
  ...
  useEffect(() => {
    const channel = supabase
      ...
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "kiosk" },
        () => fetchSlots()
      )
      .subscribe();
      ...
      <div className="mb-4 border rounded-lg p-3 bg-muted/20">
        ...
        {availableSlots > 0 ? (
          <div className="text-sm text-green-600 font-medium">
            🟢 Kiosk Available for Returns ({availableSlots} slot
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
          {slots.map((s) => {
            ...
            return (
              <div
                key={s.compartment}
                className={`border-2 rounded-[14px] p-4 ${
                  occupied
